...
@njit(fastmath=FASTMATH)
def get_month_t_boundaries(static_instance: ScenarioParameters_InstanceType, month: int64) -> UniTuple(int64, 2):
    """
    Get the first and last time interval for a month in the modelling horizon. The first time interval of
    each month is stored in the month_first_t array of the ScenarioParameters instance.

    Parameters:
    -------
    static_instance (ScenarioParameters_InstanceType): An instance of the ScenarioParameters jitclass. All of these
        parameters are static and should not be modified during unit committment.
    month (int64): Index for the month, with indexation starting at the first month of the modelling horizon.
    ...
    if month < static_instance.month_count - 1:
        ...
        last_t = static_instance.intervals_count
    return static_instance.month_first_t[month], last_t
