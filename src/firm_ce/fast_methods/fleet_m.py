...
@njit(fastmath=FASTMATH)
def initialise_annual_limits(fleet_instance: Fleet_InstanceType, static_instance, year: int64, first_t: int64) -> None:
    """
...
def update_remaining_flexible_energies(
    ...
    previous_month_flag (boolean): True indicates that the time interval has crossed into the previous month (iterating backwards
        through reverse time), indicating that the remaining_energy_month_temp_reverse must be based upon the previous month's
        remaining_energy constraint.
    ...
    for generator in fleet_instance.generators.values():
        if not generator_m.check_unit_type(generator, "flexible"):
            continue
        generator_m.update_remaining_energy(
            generator, interval, resolution, forward_time_flag, previous_year_flag, previous_month_flag, static_instance
        )
    return None
