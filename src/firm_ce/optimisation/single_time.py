...
class Solution:
    ...
    def __init__(
        ...
        # Tie storage energy capacity to power * duration when duration is fixed.
        for storage in self.fleet.storages.values():
            if storage.duration > 0 and storage.candidate_e_x_idx >= 0 and storage.candidate_p_x_idx >= 0:
                power = storage.initial_power_capacity + x[storage.candidate_p_x_idx]
                x[storage.candidate_e_x_idx] = power * storage.duration
    ...
    def balance_residual_load(self) -> boolean:
        ...
        for year in range(self.static.year_count):
            ...
            fleet_m.initialise_annual_limits(self.fleet, self.static, year, first_t)
