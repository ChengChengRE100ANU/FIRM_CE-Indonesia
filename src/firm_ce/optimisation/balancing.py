...
def initialise_interval(
    ...
    static (ScenarioParameters_InstanceType): Static scenario parameters (provides month boundaries for monthly limits).
    ...
    for node in network.nodes.values():
        ...
        for idx, flexible_order in enumerate(node.flexible_merit_order):
            generator_m.set_flexible_max_t(
                fleet.generators[flexible_order], interval, resolution, idx, forward_time_flag, static
            )
...
def balance_for_period(
    ...
    for t in range(start_t, end_t):
        ...
        fleet_m.update_remaining_flexible_energies(
            solution.fleet, t, solution.static.interval_resolutions[t], True, False, False, solution.static
        )
...
def determine_precharge_energies_for_deficit_block(interval: int64, solution, year: int64) -> int64:
    ...
    current_month = solution.static.interval_month[interval] if interval < solution.static.intervals_count else 0
    ...
    while True:
        ...
        new_month = solution.static.interval_month[interval]
        previous_month_flag = new_month != current_month
...
def resolve_energy_discontinuities(
    ...
    for interval in range(first_interval_precharge, interval_after_deficit_block):
        ...
        fleet_m.update_remaining_flexible_energies(
            solution.fleet, interval, solution.static.interval_resolutions[interval], True, False, False, solution.static
        )
    return None
